...
setwd("/Users/hectorreyes/Documents/Mayra/Laboratorio6DS")
...
Según nuestro análisis, el productor es "Yosemite Home Decor". 

```{r}
productosP <- as.vector(productos$reviews.text)
emotion <- get_nrc_sentiment(productosP)
palabras <-cbind(productos$reviews.text, emotion)
value <-get_sentiment(productos$reviews.text)
mostPos <- palabras[value==max(value)]
```
